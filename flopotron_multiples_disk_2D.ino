// ==================================================================================
//   FIRMWARE PACHATRON V10 - ULTRA-OPTIMIZADO (MÁXIMA NITIDEZ, VOLUMEN Y RITMO)
// ==================================================================================
...
// Memorias de posición y límites virtuales de Moppy 2
...
// OPTIMIZACIÓN 1: Filtro anti-saturación para guardar la última nota activa por canal
byte ultimaNotaCanal[6] = {0,0,0,0,0,0};

// --- MATRIZ DE TU CABLEADO REAL (Canales 0 al 5 de Python) ---
const byte pStep[] = {2, 4, 6, 8, 12, 10}; // Pines de paso
const byte pDir[]  = {3, 5, 7, 9, 13, 11}; // Pines de dirección

// Tabla de frecuencias oficial de Moppy (Timer a 40µs)
...
void setup() {
  // Configuración del puerto serial al máximo rendimiento
  Serial.begin(115200);

  // Inicializar pines del 2 al 13
  for (int i = 2; i <= 13; i++) {
    pinMode(i, OUTPUT);
    digitalWrite(i, LOW);
  }
  ...
  // CONFIGURACIÓN DEL TIMER 1 POR HARDWARE (Base rítmica perfecta)
  cli();
  TCCR1A = 0;
  TCCR1B = 0;
  TCNT1  = 0;
  OCR1A  = 79; // Interrupción periódica síncrona ultra veloz
  TCCR1B |= (1 << WGM12); 
...
// INTERRUPCIÓN MAESTRA EN SEGUNDO PLANO
...
void togglePin(byte driveNum, byte pin, byte direction_pin) {
  ...
  // Control de rebote largo por carril (158 medios pasos)
  ...
  // Llevar registro exacto de la pista del cabezal
  ...
  // --- OPTIMIZACIÓN 2: ONDA CUADRADA SIMÉTRICA DE ALTO VOLUMEN ---
  // Alternamos el pin usando el mismo estado del ciclo actual. Al no meter delays,
  // la bobina recibe una onda limpia que aumenta el golpe magnético de la disquetera, 
  // dándole mucha más presencia y volumen físico sin desfasar el compás.
  digitalWrite(pin, currentState[driveNum]);
  currentState[driveNum] = !currentState[driveNum];
}

void loop() {
  // OPTIMIZACIÓN 3: Lector Serial Fluido (Vía bucle 'while' en vez de 'if')
  // Esto vacía el búfer serial inmediatamente, evitando retrasos en notas rápidas.
  while (Serial.available() >= 3) {
    ...
    if (canal >= 0 && canal < MAX_DRIVES) {
      if (tipoComando == 0x90 && velocidad > 0) { // Note On
        if (nota >= 24 && nota <= 85) {
          // FILTRO ANTI-SATURACIÓN: Si la nota ya está sonando en este canal, la ignoramos.
          // Solo actualizamos si es una nota nueva o si el canal estaba apagado.
          if (ultimaNotaCanal[canal] != nota || currentPeriod[canal] == 0) {
            ultimaNotaCanal[canal] = nota;
        ...
        ultimaNotaCanal[canal] = 0; // Liberar canal para la próxima nota
      }
    }
  }
}
...
void resetAll() {
  ...
  // Calibración a fondo mecánico simultánea
